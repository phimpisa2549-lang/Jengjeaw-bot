...
                    await ctx.send(f"เพิ่มเพลง **{title}** ลงในคิวแล้วคับ! (รอเพลงแรกจบก่อนนะ)")
                else:
...
                await ctx.send("หาเพลงไม่เจอครับ ลองพิมพ์ชื่อเพลงใหม่อีกทีนะคับ")

# --- คำสั่งใหม่: ดูคิวเพลง ---
@bot.command()
async def queue(ctx):
    if not song_queue:
        await ctx.send("ตอนนี้ยังไม่มีคิวเพลงครับ 🎶")
    else:
        # แสดงรายการเพลงในคิว
